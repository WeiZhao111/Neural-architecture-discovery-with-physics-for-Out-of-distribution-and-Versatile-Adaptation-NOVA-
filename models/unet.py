...
class UNET(BaseNet):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.classifier = nn.Linear(filters[0], nclass)
    ...
    def forward(self, x):
        ...
        outputs = self.unet_head(features_final)

        # out = self.global_pooling(features_final)
...
class UNET_PINN(BaseNet):
    ...
    def __init__(
        ...
        **kwargs
        ...
        super(UNET_PINN, self).__init__(nclass, aux, norm_layer=nn.BatchNorm2d, **kwargs)
        ...
        self.block1_up = torch.nn.ConvTranspose2d(in_channels=filters[3],
    ...
    def forward(self, x):
        ...
        # Right part of the U figure in the Unet paper
        features_8s_up = self.block1_up(features_8s_down)
        features_8s_up = torch.cat([features_4s_down, features_8s_up], dim=1)

        # features_4s_up = self.block2_up(features_8s_up)
        # features_4s_up = torch.cat([features_4s_down, features_4s_up], dim=1)

        features_2s_up = self.block3_up(features_8s_up)
        features_2s_up = torch.cat([features_2s_down, features_2s_up], dim=1)
