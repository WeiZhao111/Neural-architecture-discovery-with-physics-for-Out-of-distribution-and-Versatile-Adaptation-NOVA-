...
def data_to_array(base_path, store_path, img_rows, img_cols):
    ...
    val_list = [5, 15, 25, 35, 45, 0, 10, 20, 30, 40]
...
def load_train_all_data(store_path):
    X_train_all = np.load(os.path.join(store_path, 'X_train_all.npy'))
    y_train_all = np.load(os.path.join(store_path, 'y_train_all.npy'))
...
class Promise12(BaseDataset):
    ...
    def __init__(self, root, split='train', mode=None):
        ...
        if mode == 'train':
            ...
            self.X_train_all, self.y_train_all = load_train_all_data(data_path)
            self.size = self.X_train_all.shape[0]
    ...
    def __getitem__(self, index):
        ...
        if self.mode == "train":
            ...
            img, target = self.X_train_all[index], self.y_train_all[index]
