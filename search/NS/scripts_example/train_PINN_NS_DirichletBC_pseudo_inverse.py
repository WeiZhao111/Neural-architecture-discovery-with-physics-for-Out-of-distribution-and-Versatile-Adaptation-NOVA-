...
from models.diffusion_related import Index_Transformation, Index_Transformation_01, Index_Transformation1
...
# # down sampling
# u_inner_s_all = np.zeros((0, 64, 64))
# v_inner_s_all = np.zeros((0, 64, 64))
# p_inner_s_all = np.zeros((0, 64, 64))
# t_inner_s_all = np.zeros((0, 64, 64))
# ls_inner_s_all = np.zeros((0, 64, 64))
...
# outputs_pi = np.load('/home/weiz/Astar_Work/NAS/NAS-Unet-PINN2/search/NS/NS_pi_128_kernel7.npz')['arr_0']
# # outputs_conv2d = np.load('/home/weiz/Astar_Work/NAS/NAS-Unet-PINN3/search/NS/u_net_128.npz')['arr_0']
# ind = x_ind_bc.repeat(3, axis=1)
# ind = ind[45:50, :, :, :]
# outputs_pi[ind == 30] = 0
# # outputs_conv2d[ind == 30] = 0
# labels = y_train[45:50, :, :, :]

# # visualize simulation data
# # case 1
# # norm_u0 = matplotlib.colors.Normalize(vmin=0, vmax=1.8)
# # norm_v0 = matplotlib.colors.Normalize(vmin=-0.8, vmax=0.32)
# # norm_p0 = matplotlib.colors.Normalize(vmin=-0.6, vmax=3.6)

# # case 2
# # norm_u0 = matplotlib.colors.Normalize(vmin=0, vmax=2.1)
# # norm_v0 = matplotlib.colors.Normalize(vmin=-0.48, vmax=0.8)
# # norm_p0 = matplotlib.colors.Normalize(vmin=-0.5, vmax=3.0)

# # case 3
# # norm_u0 = matplotlib.colors.Normalize(vmin=0, vmax=2.1)
# # norm_v0 = matplotlib.colors.Normalize(vmin=-0.8, vmax=0.6)
# # norm_p0 = matplotlib.colors.Normalize(vmin=-0.6, vmax=4.2)

# # case 4
# # norm_u0 = matplotlib.colors.Normalize(vmin=0, vmax=2.1)
# # norm_v0 = matplotlib.colors.Normalize(vmin=-0.48, vmax=0.48)
# # norm_p0 = matplotlib.colors.Normalize(vmin=-1.0, vmax=3.0)

# # case 5
# norm_u0 = matplotlib.colors.Normalize(vmin=0, vmax=2.1)
# norm_v0 = matplotlib.colors.Normalize(vmin=-0.96, vmax=0.16)
# norm_p0 = matplotlib.colors.Normalize(vmin=-1.2, vmax=1.6)
...
# # # u
# # ax1 = fig.add_subplot(1,3,1)
# # u0 = outputs_conv2d[0, 0, :, :]
# # plt.contourf(u0, con_lv, origin='lower', cmap='rainbow', extent=ext, aspect='auto', norm=norm_u0);
# # plt.colorbar(); plt.xlabel('x'); plt.ylabel('y')
# # ax1 = fig.add_subplot(1,3,2)
# # v0 = outputs_conv2d[0, 1, :, :]
# # plt.contourf(v0, con_lv, origin='lower', cmap='rainbow', extent=ext, aspect='auto', norm=norm_v0);
# # plt.colorbar(); plt.xlabel('x'); plt.ylabel('y')
# # ax1 = fig.add_subplot(1,3,3)
# # p0 = outputs_conv2d[0, 2, :, :]
# # plt.contourf(p0, con_lv, origin='lower', cmap='rainbow', extent=ext, aspect='auto', norm=norm_p0);
# # plt.colorbar(); plt.xlabel('x'); plt.ylabel('y')
# # plt.show()

# labels_u = labels[num, 0:1, :, :][ind[num, 0:1, :, :] < 30]
# labels_v = labels[num, 1:2, :, :][ind[num, 0:1, :, :] < 30]
# labels_p = labels[num, 2:3, :, :][ind[num, 0:1, :, :] < 30]
# outputs_pi_u = outputs_pi[num, 0:1, :, :][ind[num, 0:1, :, :] < 30]
# outputs_pi_v = outputs_pi[num, 1:2, :, :][ind[num, 0:1, :, :] < 30]
# outputs_pi_p = outputs_pi[num, 2:3, :, :][ind[num, 0:1, :, :] < 30]
# # outputs_conv2d_u = outputs_conv2d[num, 0:1, :, :][ind[num, 0:1, :, :] < 30]
# # outputs_conv2d_v = outputs_conv2d[num, 1:2, :, :][ind[num, 0:1, :, :] < 30]
# # outputs_conv2d_p = outputs_conv2d[num, 2:3, :, :][ind[num, 0:1, :, :] < 30]
# mse_pi_u = np.mean((outputs_pi_u - labels_u)**2)
# mse_pi_v = np.mean((outputs_pi_v - labels_v)**2)
# mse_pi_p = np.mean((outputs_pi_p - labels_p)**2)
# # mse_conv2d_u = np.mean((outputs_conv2d_u - labels_u)**2)
# # mse_conv2d_v = np.mean((outputs_conv2d_v - labels_v)**2)
# # mse_conv2d_p = np.mean((outputs_conv2d_p - labels_p)**2)


# outputs_pi_all = outputs_pi[num, :, :, :][ind[num, :, :, :] < 30]
# labels_all = labels[num, :, :, :][ind[num, :, :, :] < 30]
# mse_pi_all = np.mean((outputs_pi_all - labels_all)**2)
...
def main(x_inputs, genome, epochs, save='Design_1', expr_root='search', seed=0, gpu=0, init_channels=24, depth=5, lamda=1):
    ...
    #     # valid_RMSE, valid_nRMSE, valid_cRMSE, valid_max_error, valid_bRMSE = infer(valid_queue, model)
    #     # logging.info('valid RMSE: %f, valid nRMSE: %f, valid cRMSE: %f, valid bRMSE: %f, valid max error: %f', valid_RMSE, valid_nRMSE, valid_cRMSE, valid_bRMSE, valid_max_error)
    ...
    # load model
    # checkpoint = torch.load('/home/weiz/Astar_Work/NAS/NAS-Unet-PINN5/search/NS/scripts_example/train_model_128_kernel5.pt')
    # checkpoint = torch.load('/home/weiz/Astar_Work/NAS/NAS-Unet-PINN5/search/NS/scripts_example/train_model_128_kernel7.pt')
    checkpoint = torch.load('/home/weiz/Astar_Work/NAS/NAS-Unet-PINN5/search/NS/scripts_example/train_model_epoch_0.pt')
    # checkpoint = torch.load(join(save_path, 'latest_model.pth'))
    model.load_state_dict(checkpoint['model_state_dict'])
    mse = infer(valid_queue, model)
...
def infer(valid_queue, net):
    ...
    with torch.no_grad():  # change requires_grad to False
        for data, label in valid_queue:
            ...
            # data0 = np.load('/home/weiz/Astar_Work/NAS/NAS-Unet-PINN5/search/NS/scripts_example/PI_denoising_results/PI_denoising_step_0.npz')['arr_0']
            # data = torch.tensor(data0).to(device)
            ...
            # data = Index_Transformation(data)
            ...
            outputs, outputs_conv2d, ssr = net(data)
            # outputs = net(data)
